##########################################
###  SMDE R Lab: ANOVA ###################
#### Nihan Acar-Denizli, Phd  ############
#### UPC, Barcelona ######################
##########################################

### Generating Normal random vectors
v1=rnorm(200, mean=0, sd=1)
v2=rnorm(200, mean=2, sd=1)
v3=rnorm(200, mean=0, sd=1)

plot(density(v1),xlim=c(-4,6),ylim=c(0,0.5), main="Three Normal distributions")
#2 dis with same mean, 2nd with high dis around 2
#
lines(density(v2),col=2)
lines(density(v3),col=3)

v1n=data.frame(x1=v1, x2="v1")
v2n=data.frame(x1=v2, x2="v2")
v3n=data.frame(x1=v3, x2="v3")

install.packages("RcmdrMisc")
library(RcmdrMisc)
?mergeRows
#cannot merge all 3 at once. need two steps
#merge is same as rbind but rbind unifies vectors. here we are unig=fying dataframes. easier to use mergerows
#just for 2 variables can use rbind
data=mergeRows(v1n, v2n, common.only=FALSE)
data=mergeRows(as.data.frame(data), v3n, common.only=FALSE)
head(data)
summary(data)
### ANOVA on simulated data ####
AnovaModel.1 <- aov(x1 ~ x2, data=data)
summary(AnovaModel.1)
#3 stars means this model is sig at almost 100%. least diff bw means of the 3 groups
#obj of anova is to check if means are equal or not for the 3 groups
#Ho all means equal
#H1 atlest one pair has diff means

library(car)
Boxplot(x1~x2,data=data,id=FALSE)
#we rejected null with more than 95%
#g2 seems to have higher median and mean comparng to other groups. sig diff due to the distribution of the second group

#install.packages("lmtest")
library(lmtest)

##### Assumptions of ANOVA ###
#The observations within each sample must be independent.
#Durbin Watson 
dwtest(AnovaModel.1, alternative ="two.sided")
#null hyp says that autocorr is 0
#obs are independent
#The populations from which the samples are selected must have equal variances (homogeneity of variance)
#Breusch Pagan test
#looking d=for correlation bw obs. if we dont have then we are right
#The populations from which the samples are selected must be normal.
#Shapiro test
shapiro.test(residuals(AnovaModel.1))
#rsiduals and ... sould follow he nirmal dis. shape should not be same but dis should be normal
bptest(AnovaModel.1)
#to test in more than 2 geoups we cannot use wartest as it can test for max 2
#hence using bptest
#shows p value greater than 0.05

##################################################
######## ANOVA Example: Wine (FactoMineR) ########
##################################################

#install.packages("FactoMineR")
library(FactoMineR)
# will also be used in principle... in next classs
data(wine)
head(wine)
summary(wine)

#### Assumptions ####
#### Normality of quantitative variables ###

# Histograms #
hist(wine$Aroma.intensity)
#left skewed
hist(wine$Flower)
#symmetrical but one outlier which affects the dis. 
#can either omit but need to give an explanation why
#will give good resuls
hist(wine$Intensity)
#skewed dis to the left. we see 2 outlying obs
#can apply analysis but since there is no normality it will affect
#if you have a right skewed u can normalize using look tranformation
#need to search for what type of trandsormtion will be suitable for his shape

# Test of Normality #
shapiro.test(wine$Aroma.intensity)
shapiro.test(wine$Flower)
shapiro.test(wine$Intensity)

#all 3 seeema normal. but from hist we see high amt of skewness which is due to a few obs (2 here)
#if eliminating, explain why. why these nums are diff from the rest of the dist, or wy they show dif charact, due to structure of the dist we are not including

## ANOVA MODELS ##
model1<-aov(Aroma.intensity~Soil,data=wine)
summary.aov(model1)
#p>0.05 means we accept null hypotesis where means are all equal
model2<-aov(Flower~Soil,data=wine)
summary.aov(model2)

model3<-aov(Intensity~Soil,data=wine)
summary.aov(model3)
#intensity shows they have diff means acc to intensity, reject hyp
model4<-aov(Intensity~Label,data=wine)
summary.aov(model4)

#label is not an imp var for intensity of the wone
#but 
Boxplot(wine$Intensity~wine$Soil,id=FALSE,col=2:(nlevels(wine$Soil)+1))

#### Independency of observations ###
dwtest(model3,alternative="two.sided")
#checking autocoor, null not independent
#since >0.05 i accept the indepencny of obs

### Homogenity of Variances #####
### Levene's Test #####

?leveneTest
#leveneTest(Flower~Soil,data=wine)

leveneTest(Intensity~Soil,data=wine)
#nums cannot be exactly same since they use diff formulas but obs should be the same
#(results) levene-breusch
### Breusch Pagan test ####
bptest(model3)
#2 grps have diff var, we are rej null hyp
### Normality of residuals ###
shapiro.test(model3$residual)
#shows th4e residuals of anther==r model 
#########################################
########## Multiple Comparisons #########
#########################################

#### Tukey's HSD ####
?TukeyHSD
#baseeed on fitted model
#by default 0.95%

TukeyHSD(model3)
#acc to p values we see a sig diff bw env1-ref
#which grp has higher intentsity
#minus because ref mean is higher
#
#### Pairwise t-test ###
?pairwise.t.test

with(wine,
{
  pairwise.t.test(Intensity,Soil,p.adj="none")
})

#by doing this analysis we inc type 1 error qhich we dont want
######## Bonferroni Correction ###
 
with(wine,
{ 
pairwise.t.test(Intensity,Soil,p.adj="bonf")
})

####### LSD Test ####
install.packages("agricolae")
library(agricolae)

?LSD.test
LSD<-LSD.test(model3,"Soil") #p.adj = "bonferroni")
LSD

###########################
####### Two-Way ANOVA #####
###########################

model5<-aov(Intensity~Soil+Label,data=wine)
summary(model5)
# find a sig diff bc 1 and ref confidence level not overlpping- 3.22-3.23 no overlap
head(wine)
#confident levels gives same results
#for proj look for whoch is sig effect
for (i in 3:31){
  print(colnames(wine)[i])
  print(summary(aov(wine[,i]~Soil+Label,data=wine)))
}

model6<-aov(Odor.Intensity~Soil + Label,data=wine)
summary(model6)

dwtest(model6)
#normality
shapiro.test(wine$Odor.Intensity)
hist(wine$Odor.Intensity)
#homogenity of var
leveneTest(Odor.Intensity~Soil,data=wine)
leveneTest(Odor.Intensity~Label,data=wine)
#post hoc test
TukeyHSD(model6)
#model7<-aov(Odor.Intensity~soil+label+soil)
## Interaction Plot ###
?interaction.plot
interaction.plot(wine$Soil,wine$Label, wine$Odor.Intensity, fun=mean, ylim=c(3,4), type="l", legend=TRUE)

#####################################################################
############### EMPLOYEE SATISFACTION DATA SET ######################
#####################################################################

posdep<-read.csv2("posdep.csv")
#need to convert to factors
posdep<-posdep[,-1]

str(posdep)
colnames(posdep)
summary(posdep)

posdep$Position<-as.factor(posdep$Position)
posdep$Department<-as.factor(posdep$Department)
summary(posdep)

### Affect of Position and Department on Satisfaction ###
model_sat <- aov(Satisfaction ~ Department*Position,data=posdep)
summary.aov(model_sat)

######################
### Post Hoc Tests ###
######################

## Position ##
sat_pos <- aov(Satisfaction ~ Position, data = posdep)
TukeyHSD(sat_pos, "Position")

### Department ## 
sat_dep <- aov(Satisfaction ~ Department, data = posdep)
TukeyHSD(sat_dep, "Department")

### Interaction Plots for Position and Department ###

interaction.plot(posdep$Position,posdep$Department, posdep$Satisfaction, fun=mean, type="l", legend=TRUE,col = 1:5)
interaction.plot(posdep$Position,posdep$Department,posdep$Ownage,fun=mean,type="l",legend=TRUE,col=1:5)
interaction.plot(posdep$Position,posdep$Department,posdep$Permanence,fun=mean,type="l",legend=TRUE,col=1:5)

#####################################################################################

### Multivariate ANOVA (MANOVA) ###
posdep_man <- manova(cbind(Satisfaction, Ownage, Permanence) ~ Position*Department,data=posdep)
summary.aov(posdep_man)

## Multiple Comparisons ##
## For Position ## 
#manova - multiple anova
    TukeyHSD(aov(Satisfaction~Position,data=posdep),"Position")  
    TukeyHSD(aov(Ownage~Position,data=posdep),"Position")
    TukeyHSD(aov(Permanence~Position,data=posdep),"Position")

    ## For Department ##  
    TukeyHSD(aov(Satisfaction~Department,data=posdep),"Department")  
    TukeyHSD(aov(Ownage~Department,data=posdep),"Department")
    TukeyHSD(aov(Permanence~Department,data=posdep),"Department")

    
        